...
export async function createCompetition(input: CreateCompetitionInput): Promise<CreateCompetitionOutcome> {
  ...
  const requestBody = compactPayload({
    ...
    // the field's own doc comment on CreateCompetitionInput above. Not
    // routed through the `|| undefined` normalisation below since it has no
    // "empty means absent" reading — always a real, non-empty enum value.
